...
export class GradeService {
  ...
  public async getGradesByHomeworkSubmissionId(homeworkSubmissionId: string) {
    try {
      const response = await apiClient.get<Group>(
        `/grade/homeworkSubmission/${homeworkSubmissionId}`,
      );
      return response.data;
    } catch (error) {
  ...
  public async getGradeByHomeworkSubmissionIdAndStudentId(
    homeworkSubmissionId: string,
    studentId: string,
  ) {
    try {
      const response = await apiClient.get<Group>(
        `/grade/homeworkSubmission/${homeworkSubmissionId}/student/${studentId}`,
      );
      return response.data;
    } catch (error) {
