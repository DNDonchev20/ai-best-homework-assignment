...
async function fetchGroupsAndStudents() {
  try {
    ...
    groups.value = fetchedGroups.filter(group => group !== null && group !== undefined);

    // Fetch all students for the groups
    const allStudentDetails: StudentDetail[] = [];
    const studentPromises = groupIds.map(async (groupId) => {
      const studentDetails = await studentDetailsService.findStudentsByGroupId(groupId);
    ...
    const userIds = Array.from(new Set(allStudentDetails.map(student => student.userId)));
...
<template>
  ...
  <div class="mt-20 px-52">
    <ClassComponent :groups="groups" :students="students" :subject="subject" :user="user" />
  </div>
</template>
