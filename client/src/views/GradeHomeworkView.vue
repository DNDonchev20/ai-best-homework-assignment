...
const fetchHomework = async () => {
  try {
    ...
    if (selectedSubmission.value) {
      const student = await userService.getOtherUserByUserId(
        selectedSubmission.value.studentId,
      );
      selectedSubmission.value.studentName = student.name;

      try {
        const grades = await gradeService.getGradesByHomeworkSubmissionId(
...
<template>
  ...
  <div class="container mx-auto mt-10 rounded-lg bg-gray-50 p-6 shadow-lg">
    <div class="mb-8">
      <h1 class="text-3xl font-bold text-blue-600">{{ homework?.title }}</h1>
      <p class="mt-2 text-lg text-gray-700">{{ homework?.description }}</p>
    </div>

    <div v-if="selectedSubmission" class="rounded-lg bg-white p-6 shadow-md">
      ...
      <div v-if="selectedSubmission.isGraded" class="mt-6">
        ...
        <p class="text-lg text-gray-700">
          <strong>Percentage:</strong> {{ gradeData.percentage }}%
        </p>
        <p class="text-lg text-gray-700">
        ...
        <button
          @click="deleteGrade"
          class="mt-4 w-full rounded-md bg-red-600 py-3 text-white hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500"
        >
          Delete Grade
        </button>
      </div>
      ...
      <div v-else class="mt-6">
        ...
        <form @submit="postGrade">
          <div class="mb-4">
            <label for="grade" class="block text-lg font-medium text-gray-700"
              >Grade</label
            >
            <input
              ...
              class="mt-2 w-full rounded-md border border-gray-300 p-3 shadow-sm"
            />
          </div>
          ...
          <div class="mb-4">
            ...
            <textarea
              ...
              class="mt-2 h-32 w-full rounded-md border border-gray-300 p-3 shadow-sm"
            ></textarea>
          </div>

          <button
            type="submit"
            class="w-full rounded-md bg-blue-600 py-3 text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500"
          >
            Submit Grade
          </button>
        </form>
