...
const fetchGrade = async () => {
  try {
    const fetchedGrade = await gradeService.getGradeByHomeworkSubmissionIdAndStudentId(submission.value.id, studentId.value);
    if (fetchedGrade) {
...
<template>
    ...
    <div class="container mx-auto mt-10 max-w-[700px] rounded-lg border-2 border-solid p-6 shadow-lg">
      <div class="mb-8">
      ...
      <div v-if="submissionExists && submission" class="bg-white p-6 rounded-lg shadow-md mb-6">
        ...
        <div v-if="grade" class="mt-4">
          ...
          <p class="text-lg text-gray-700"><strong>Grade:</strong> {{ grade.grade }}</p>
          <p class="text-lg text-gray-700"><strong>Percentage:</strong> {{ grade.percentage }}%</p>
          <p class="text-lg text-gray-700"><strong>Feedback:</strong> {{ grade.feedback }}</p>
        </div>
        ...
        <button
          ...
          class="w-full mt-4 bg-red-600 text-white py-2 rounded-md hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500"
        >
          Delete Submission
        </button>
      </div>
