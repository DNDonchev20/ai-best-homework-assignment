...
const fetchSubmissionDetails = async (submissionId: string) => {
  try {
    submission.value = await homeworkSubmissionService.getHomeworkSubmissionById(submissionId);
  } catch (err) {
...
const submitHomework = async (event: Event) => {
  ...
  try {
    ...
    await fetchSubmissionDetails(submissionResponse.id);
...
<template>
    <div class="container mx-auto mt-10 p-6 bg-gray-50 rounded-lg shadow-lg">
      ...
      <div v-if="submission" class="bg-white p-6 rounded-lg shadow-md mb-6">
